...
# Compared to enhanced flow, the baseline system prompt just translates outline to speech
...
Your task is to write a speech script using a content_outline.
...
2. Follow the structure of the content_outline.
...
def build_writer_user_prompt(content_outline):
    prompt = f"""
Write a speech script using the content outline below.
...
def Script_Writing_Agent(state: SpeechScriptState):
    """Generate a speech from an outline"""
    print("\n========SCRIPT WRITING AGENT ========")
    llm_script = speech_writing_llm.invoke([
        SystemMessage(content=writer_system_prompt),
        HumanMessage(
            content=build_writer_user_prompt(content_outline=state["content_blueprint"])
        )
    ])
