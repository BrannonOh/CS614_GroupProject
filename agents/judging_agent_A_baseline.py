...
from prompts.judging_agent_baseline import JUDGING_SYSTEM_PROMPT, build_judging_user_prompt 
...
def judging_agent_a_node(state: SpeechScriptState):
    ...
    if stylistic_script is None:
        return {
            "judge_a_result": None,
            "judge_a_error": "Judging agent (A) called without final_speech.",
        }

    if planner_blueprint is None:
        return {
            "judge_a_result": None,
            "judge_a_error": "Judging agent (A) called without planner_blueprint.",
        }
    ...
    try:
        ...
        return {
            "judge_a_result": None,
            "judge_a_error": f"Pydantic validation failed: {str(e)}"
        }        
        ...
        return {
            "judge_a_result": None,
            "judge_a_error": f"Judging agent (A) failed: {str(e)}"
        }
